...
def _format_proposal_for_match(p: ResearchProposal, label: str) -> str:
    """
    把 proposal 渲染成 pairwise compare 的 prompt 片段。

    R10-Pri-1 fix: 之前只喂 motivation + proposed_method，feasibility 维度的 LLM
    判断完全是瞎猜（看不到 phase 设计、compute hours、technical_details）。补上
    phases 简表 + tech_details 摘要后，feasibility 比较才有真依据。
    """
    phase_lines = []
    for ph in p.methodology_phases:
        phase_lines.append(
            f"  P{ph.phase_number} ({ph.expected_compute_hours}h): "
            f"{ph.name} — {ph.description[:120]}"
        )
    phases_block = "\n".join(phase_lines) if phase_lines else "  (no phases)"
    return (
        f"【proposal {label}】\n"
        f"title: {p.title}\n"
        f"motivation (摘): {p.motivation[:500]}\n"
        f"proposed_method (摘): {p.proposed_method[:500]}\n"
        f"technical_details (摘): {p.technical_details[:300]}\n"
        f"phases (total {p.total_estimated_hours}h, fits_budget={p.fits_resource_budget}):\n"
        f"{phases_block}"
    )
...
def pairwise_compare(
    ...
    user_msg = (
        f"{_format_proposal_for_match(proposal_a, 'A')}\n\n"
        f"{_format_proposal_for_match(proposal_b, 'B')}\n\n"
...
def compute_elo_rankings(
    ...
    *,
    ...
    R10-Pri-5 fix: Elo 是 order-sensitive 的（同一组胜负，处理顺序不同会得到
    不同分数）。N=5 时单 pass 误差 ~20 Elo，可能影响 top-1 选择。改成跑
    n_passes 轮 shuffle 后取平均 rating，方差大幅降低。
    Win/loss/tie 累计与 pass 顺序无关，直接用一份。

    Args:
        proposal_ids: 全部参赛 id
        matches: pairwise 结果（顺序不重要，本函数会 shuffle）
        n_passes: 平均轮数（默认 5；越大越稳，但 ratings 收敛后 diminishing return）
        seed: 复现用，固定 seed 保证 same input → same output
    ...
    # win/loss/tie 累计（与 pass 顺序无关）
    stats = {pid: {"wins": 0, "losses": 0, "ties": 0} for pid in proposal_ids}
    valid_matches = [
        m for m in matches
        if m.proposal_a_id in stats and m.proposal_b_id in stats
    ]
    ...
    # n_passes 轮 shuffle，每轮独立计算 ratings，最后求平均
    rng = _random.Random(seed)
    accum = {pid: 0.0 for pid in proposal_ids}
    n_eff = max(1, n_passes)
    for _ in range(n_eff):
        ratings = {pid: _DEFAULT_ELO_START for pid in proposal_ids}
        order = list(range(len(valid_matches)))
        rng.shuffle(order)
        for idx in order:
            m = valid_matches[idx]
            _update_elo(ratings, m.proposal_a_id, m.proposal_b_id, m.winner)
        for pid in proposal_ids:
            accum[pid] += ratings[pid]

    rankings = []
    for pid in proposal_ids:
        rankings.append({
            "proposal_id": pid,
            "elo": round(accum[pid] / n_eff, 1),
            **stats[pid],
        })
